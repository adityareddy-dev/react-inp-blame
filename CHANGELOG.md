...
- **A setup for React Router in framework mode and for TanStack Start.** Both write their own HTML, so the
  ...
  On React 19 the setup should hold there too. On React 18 it does not, since that `react-dom` connects to
  React's DevTools hook as it loads, and a route can import it before the entry does.
