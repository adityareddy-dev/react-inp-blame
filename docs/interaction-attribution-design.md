...
637 components inside TableBody, 257 ms of it in TableBody's own render rather than the components under
it", `blame.detail` is "TableBody's own render", and a note says that is work done while rendering,
which memoising the rows does not speed up. `explanation.blame.confidence` says what the
