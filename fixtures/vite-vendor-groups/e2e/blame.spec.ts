...
// fixtures/vite-react-ts on Vite 8 and React 18, whose react-dom connects to the DevTools hook as it
// loads, with every dependency sent to one vendor chunk by a Rolldown codeSplitting group, and Radix's
// Portal, which imports react-dom, in the page. In a build the page's install script imports the library;
// if the library sat in the vendor chunk, the Portal's module would run react-dom as that chunk loaded,
// before install(), and nothing would be blamed. The counter re-renders SlowList's 400 rows, so a click on
// it is slow.
test('a click is blamed on SlowList', async ({ page }) => {
  ...
  try {
    ...
    await expect(page.locator('#portal-note')).toHaveText('Rendered through a portal');
